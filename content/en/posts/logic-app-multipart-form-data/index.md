...
Start by creating a new workflow in the existing Logic App. 
To send a `multipart/form-data` payload with Logic Apps we need to use the HTTP connector and compose a similar body that was seen in the [Read the data](#read-the-data) section, a `json` body with the `$multipart` & `$content-type` properties and pass in the the content in array.
...
```
Notice that we did not set the `boundary` value since this gets auto generated by HTTP connector. The same goes for the `Content-type` header, from what I have tested it does not matter when working with multipart/from-data. 
